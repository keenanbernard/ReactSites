...
import yearlyBanner from "./Nomination Images/Digi RR Yearly.png";
import FormCategories from "./FormCategories/FormCategories";
import MessageDetails from "./FormCategories/MessageDetails";

function YearlyNomination(props) {
    const {accounts} = useMsal();
    const name = accounts[0] && accounts[0].name;
    const [users, setUsers] = useState([]);
    const [salesP, setSalesP] = useState(false);
    const [creditP, setCreditP] = useState(false);
    ...
    const [nominationCategoryIsValidOnSub, setNominationCategoryIsValidOnSub] = useState(false);
    const [commentSizeIsInvalidOnSub, setCommentSizeIsInvalidOnSub] = useState(false);


    // Form Validation
    const yearlyNomineeIsValid = selectedValue !== '';
    ...
    if (yearlyNomineeIsValid && nominationCategoryIsValid && nominatorCommentIsValid && justificationFileIsValid && fileTypeAccepted && fileSizeAccepted && commentSizeIsValid) formIsInvalid = false;
    ...
    const yearlyNomineeChangeHandler = value => {
    ...
    const getNominationCategory = (event) => {
        setNominationCategoryIsValidOnSub(false);
    ...
    if (selectedValue !== null) nomineeEmail = selectedValue.mail;
    ...
    const getNominations = (event) => {
        ...
        const subject = 'Digi Annual Award!';
        ...
        const nomineeMessage = `We are excited to announce that you have been nominated by ` + name + ` for this Fiscal Years ` + qacat + ` Annual Award. Winners will be announced on the I AM DIGI Recognition Portal.`;

        if (!yearlyNomineeIsValid) setSelectedValueIsInvalidOnSub(true);

        if (!nominationCategoryIsValid) setNominationCategoryIsValidOnSub(true);
        ...
        if (!yearlyNomineeIsValid || !nominatorCommentIsValid || !justificationFileIsValid || !nominationCategoryIsValid || !commentSizeIsValid) return;
        ...
        fetch(`${props.host}/getDivision?id=${selectedValue.id}`)
            ...
            fetch(`${props.host}/InsertYearlyNominations`, {
                ...
                if (response.status === 200) {
                    NotificationManager.success('You have successfully sent a Digi Annual Nomination', 'SUCCESS');
                    axios.post(`${props.host}/EmailNotification`, {recieverEmail: nomineeEmail,  peerMessage: nomineeMessage, subject: subject
                        ...
                        NotificationManager.error('Unable to send Annual Notification, Contact Admin', 'ERROR: ' + err, 5000);
                    })
                }
            }).catch(err => {
                NotificationManager.error('Unable to send Annual Nomination, Contact Admin', 'ERROR: ' + err, 5000);
            });
        }).catch(err => {
    ...
    const digiCEOSCircle = [
        { label: `Digi CEO's Circle`, value: `Digi CEOs Circle`}]
    const digiChairmansSelection = [
        { label: `Digi Chairman's Selection`, value: `Digi Chairmans Selection`}]
        ...
        <Modal {...props} size="lg" aria-labelledby="contained-modal-title-vcenter" centered>
            <Modal.Header className='quarterlyBanner' style={{padding: 0}}>
                <Modal.Title id="contained-modal-title-vcenter">
                    <img className='' src={yearlyBanner} alt={'Quarterly Awards'} style={{width: '100%'}} />
                </Modal.Title>
            </Modal.Header>
            <Modal.Body>
                <Row>
                    <Col md={12}>
                        <div>
                            ...
                            <Select
                                ...
                                onChange={yearlyNomineeChangeHandler}
                                options={users && users}
                            />
                        ...
                        <FormCategories type='radio' id='CorporateBehaviors' name='corporateCategories' corpTrait={digiCEOSCircle[0].label} message=' - Quarterly award winners will have automatic entry to Annual Awards. Newly Nominated employees will also be eligible to win the annual award.' value={digiCEOSCircle[0].value} onChange={getNominationCategory} />

                        <FormCategories type='radio' id='CustomerService' name='corporateCategories' corpTrait={digiChairmansSelection[0].label} message= ' - Chairman selection of an outstanding leader for the year. Criteria to be determined by the chairman.' value={digiChairmansSelection[0].value} onChange={getNominationCategory} />
                        ...
                        {nominationCategoryIsValidOnSub && <p className='mt-1 error-text'>A category must be selected</p>}
...
export default YearlyNomination;
